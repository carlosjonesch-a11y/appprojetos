...
def create_projeto_card(projeto, col):
    """Cria um card visual para projeto"""
    with col:
        st.markdown(f"""
        <div style="
...
def create_demanda_card(demanda: Demanda, col, on_click_edit: Optional[Callable] = None, on_click_delete: Optional[Callable] = None):
    """Cria um card visual para demanda com opções de interação"""
    prioridade_cores = {
        "Baixa": "#4CAF50",
        "Média": "#FF9800", 
        "Alta": "#f44336",
    ...
    with col:
        ...
        ">
            ...
            <div style="display: flex; gap: 8px; margin-top: 10px;">
                <span style="background: {cor_status}; color: white; padding: 4px 8px; border-radius: 4px; font-size: 0.8em;">
                    {demanda.status}
                </span>
                <span style="background: {cor_prioridade}; color: white; padding: 4px 8px; border-radius: 4px; font-size: 0.8em;">
                    {demanda.prioridade}
                </span>
            </div>
        ...
        with col1:
            if st.button("✏️ Editar", key=f"edit_{demanda.id}", width="stretch"):
                if on_click_edit:
                    on_click_edit(demanda)
        with col2:
            if st.button("🗑️ Deletar", key=f"delete_{demanda.id}", width="stretch"):
                if on_click_delete:
                    on_click_delete(demanda)

def create_demanda_form(projetos: List, etapas: List, demanda: Optional[Demanda] = None):
    """Cria um formulário para criar/editar demanda"""
    ...
    with col1:
        titulo = st.text_input(
            "Título",
            value=demanda.titulo if demanda else "",
    ...
    with col2:
        projeto_id = st.selectbox(
            "Projeto",
            options=[p.id for p in projetos],
            format_func=lambda x: next((p.nome for p in projetos if p.id == x), x),
            index=next((i for i, p in enumerate(projetos) if p.id == demanda.projeto_id), 0) if demanda else 0,
            key="form_projeto_selectbox"
        )
    
    descricao = st.text_area(
        "Descrição",
        value=demanda.descricao if demanda else "",
        placeholder="Descreva a demanda em detalhes",
        height=100,
        key="form_descricao_textarea"
    )
    ...
    with col1:
        status = st.selectbox(
            ...
            index=next((i for i, s in enumerate(StatusEnum) if s.value == (demanda.status if demanda else StatusEnum.TODO.value)), 0),
            key="form_status_selectbox"
        )
    ...
    with col2:
        prioridade = st.selectbox(
            ...
            index=next((i for i, p in enumerate(PriorityEnum) if p.value == (demanda.prioridade if demanda else PriorityEnum.MEDIA.value)), 0),
            key="form_prioridade_selectbox"
        )
    
    with col3:
        etapa_id = st.selectbox(
            "Etapa",
            options=[None] + [e.id for e in etapas],
            format_func=lambda x: "Sem etapa" if x is None else next((e.nome for e in etapas if e.id == x), x),
            key="form_etapa_selectbox"
        )
    ...
    with col1:
        responsavel = st.text_input(
            "Responsável",
            value=demanda.responsavel if demanda and demanda.responsavel else "",
            placeholder="Nome da pessoa responsável",
            key="form_responsavel_input"
        )
    
    with col2:
        tags_input = st.text_input(
            "Tags (separadas por vírgula)",
            value=", ".join(demanda.tags) if demanda and demanda.tags else "",
            placeholder="backend, urgente, api",
            key="form_tags_input"
        )
    ...
    with col1:
        data_inicio_plano = st.date_input(
            "Data Início (Plano)",
            value=datetime.fromisoformat(demanda.data_inicio_plano).date() if demanda and demanda.data_inicio_plano else None,
            key="form_data_inicio_plano"
        )
    
    with col2:
        data_vencimento_plano = st.date_input(
            "Data Vencimento (Plano)",
            value=datetime.fromisoformat(demanda.data_vencimento_plano).date() if demanda and demanda.data_vencimento_plano else None,
            key="form_data_vencimento_plano"
        )
    
    # ============ DATAS REAIS ============
    st.markdown("### 📊 **Datas Reais (Real)**")
    col1, col2 = st.columns(2)
    
    with col1:
        data_inicio_real = st.date_input(
            "Data Início (Real)",
            value=datetime.fromisoformat(demanda.data_inicio_real).date() if demanda and demanda.data_inicio_real else None,
            key="form_data_inicio_real"
        )
    
    with col2:
        data_vencimento_real = st.date_input(
            "Data Vencimento (Real)",
            value=datetime.fromisoformat(demanda.data_vencimento_real).date() if demanda and demanda.data_vencimento_real else None,
            key="form_data_vencimento_real"
        )
    
    # Manter data_vencimento para compatibilidade
...
def create_etapa_form(etapa=None):
    """Cria um formulário para criar/editar etapa"""
    ...
    with col1:
        nome = st.text_input(
            "Nome da Etapa",
            value=etapa.nome if etapa else "",
            placeholder="Ex: Design, Desenvolvimento, Testes",
            key="form_etapa_nome_input"
        )
    
    with col2:
        ordem = st.number_input(
            "Ordem",
            value=etapa.ordem if etapa else 0,
            min_value=0,
            key="form_etapa_ordem_input"
        )
    
    descricao = st.text_area(
        "Descrição",
        value=etapa.descricao if etapa else "",
        placeholder="Descreva a etapa",
        height=80,
        key="form_etapa_descricao_textarea"
    )
...
def show_status_badge(status: str):
    """Exibe um badge de status com cor apropriada"""
    status_cores = {
        "A Fazer": "#90CAF9",
        "Em Progresso": "#FFB74D",
        "Em Revisão": "#CE93D8",
...
def show_priority_badge(prioridade: str):
    """Exibe um badge de prioridade com cor apropriada"""
    prioridade_cores = {
        "Baixa": "#4CAF50",
        "Média": "#FF9800",
        "Alta": "#f44336",
